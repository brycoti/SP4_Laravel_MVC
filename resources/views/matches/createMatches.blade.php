...
<div class="py-12 h-full">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-blue-600 text-white p-6 mb-6 rounded focus:outline-none">
            <h1 class="text-2xl font-semibold text-center">Schedule a New Match</h1>
        </div>
        ...
        <div class="p-4 sm:p-8 mb-6 bg-blue-100 shadow sm:rounded-lg flex items-center justify-center">
            <form method="POST" action="{{ route('matches.store') }}" class="max-w-xl">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <select id="id_home_team" name="id_home_team" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                        @foreach($teams as $team)
                            <option value="{{ $team->team_id }}" {{ old('id_home_team') == $team->team_id ? 'selected' : '' }}>
                                {{ $team->team_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <select id="id_away_team" name="id_away_team" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                        @foreach($teams as $team)
                            <option value="{{ $team->team_id }}" {{ old('id_away_team') == $team->team_id ? 'selected' : '' }}>
                                {{ $team->team_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    <input type="datetime-local" id="match_date" name="match_date" value="{{ old('match_date') }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                </div>

                <!-- Home Team Goals -->
                <div class="mb-6">
                    <label for="home_team_goals" class="block text-xl text-black-600 mb-2">Home Team Goals:</label>
                    <input type="number" name="home_goals" min="0" value="{{ old('home_goals') }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                </div>

                <!-- Away Team Goals -->
                <div class="mb-6">
                    <label for="away_team_goals" class="block text-xl text-black-600 mb-2">Away Team Goals:</label>
                    <input type="number" name="away_goals" min="0" value="{{ old('away_goals') }}" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                </div>
